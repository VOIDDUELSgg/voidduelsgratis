...
local OFF_CLR = Color3.fromRGB(60,60,60)
local MOB_ON  = Color3.fromRGB(80,80,80)
local MOB_OFF = Color3.fromRGB(15,15,15)
...
autoTPDownEnabled=false autoTPDownTargetY=20 autoTPDownConn=nil
...
aimbotHighlight.Name="VoidDuelsAimbotESP"
aimbotHighlight.FillColor=Color3.fromRGB(80,80,80)
...
local function doTPDown()
    ...
    local result=workspace:Raycast(r.Position,Vector3.new(0,-500,0),rayParams)
    local groundY=result and result.Position.Y+3 or r.Position.Y
    r.CFrame=CFrame.new(r.Position.X,groundY,r.Position.Z)
    r.AssemblyLinearVelocity=Vector3.zero
end

-- AUTO TP DOWN
local autoTPDownBaseY=nil
...
local function startAutoTPDown()
    ...
    autoTPDownConn=RunService.Heartbeat:Connect(function()
        if not autoTPDownEnabled then autoTPDownConn:Disconnect() autoTPDownConn=nil return end
        local r=getHRP() if not r then return end
        local hum=getHum() if not hum then return end
        if hum.FloorMaterial~=Enum.Material.Air then
            autoTPDownBaseY=r.Position.Y
        end
        if autoTPDownBaseY and r.Position.Y - autoTPDownBaseY >= autoTPDownTargetY then
            local groundY=getGroundY(Vector3.new(r.Position.X, autoTPDownBaseY+10, r.Position.Z))
            r.CFrame=CFrame.new(r.Position.X, groundY, r.Position.Z)
            r.AssemblyLinearVelocity=Vector3.zero
        end
    end)
    ...
    tauntLoop=task.spawn(function()
        while tauntActive do
            pcall(function()
                ...
                if ch then ch:SendAsync("/lol Void better🤣🤣")
                else game:GetService("ReplicatedStorage"):WaitForChild("DefaultChatSystemChatEvents",1):WaitForChild("SayMessageRequest",1):FireServer("/lol Void better🤣🤣","All") end
            end)
            task.wait(0.5)
        end
    end)
...
local function makeSpeedBB()
    ...
    speedBB=Instance.new("BillboardGui") speedBB.Name="VoidDuelsSpeedBB" speedBB.Adornee=head
...
local function createESP(plr)
    ...
    local head=c:FindFirstChild("Head") if not head then return end
    if c:FindFirstChild("VoidDuelsESP") then return end
    local box=Instance.new("BoxHandleAdornment")
    box.Name="VoidDuelsESP" box.Adornee=root box.Size=Vector3.new(4,6,2)
    box.Color3=ACCENT box.Transparency=0.45 box.ZIndex=10 box.AlwaysOnTop=true box.Parent=c
    local bb=Instance.new("BillboardGui")
    bb.Name="VoidDuelsESP_Name" bb.Adornee=head bb.Size=UDim2.new(0,200,0,45)
...
local function removeESP(plr)
    ...
    local b=plr.Character:FindFirstChild("VoidDuelsESP") local n=plr.Character:FindFirstChild("VoidDuelsESP_Name")
...
local function startSpin()
    ...
    spinBAV=Instance.new("BodyAngularVelocity") spinBAV.Name="VoidDuelsSpinBAV"
...
ScreenGui.Name="VoidDuels" ScreenGui.ResetOnSpawn=false
...
TitleBar.Size=UDim2.new(1,0,0,52) TitleBar.BackgroundColor3=Color3.fromRGB(15,15,15) TitleBar.BorderSizePixel=0
Instance.new("UICorner",TitleBar).CornerRadius=UDim.new(0,16)
local TitleFix=Instance.new("Frame",TitleBar)
TitleFix.Size=UDim2.new(1,0,0,16) TitleFix.Position=UDim2.new(0,0,1,-16)
TitleFix.BackgroundColor3=Color3.fromRGB(15,15,15) TitleFix.BorderSizePixel=0
local TitleLbl=Instance.new("TextLabel",TitleBar)
TitleLbl.Size=UDim2.new(1,0,1,0) TitleLbl.BackgroundTransparency=1 TitleLbl.Text="VOID DUELS"
...
do
    ...
    plusBtn.BackgroundColor3=Color3.fromRGB(30,30,30) plusBtn.Text="+"
    ...
    minusBtn.BackgroundColor3=Color3.fromRGB(30,30,30) minusBtn.Text="-"
...
DiscordLbl.BackgroundTransparency=1 DiscordLbl.Text="Void Duels"
...
local function makeTab(text,xScale,xPos)
    ...
    t.TextColor3=Color3.fromRGB(180,180,180) t.BorderSizePixel=0
...
local function selectTab(idx)
    ...
    for i,tb in ipairs(tabs) do
        ...
        tb.TextColor3=(i==idx) and Color3.fromRGB(255,255,255) or Color3.fromRGB(180,180,180)
    end
end
...
local function makeToggle(parent,name,order,callback,defaultOn)
    ...
    checkBtn.Size=UDim2.new(0,32,0,32) checkBtn.Position=UDim2.new(1,-44,0.5,-16)
    checkBtn.BackgroundColor3=initState and WHITE or OFF_CLR
    checkBtn.Text=initState and "✓" or ""
    checkBtn.TextColor3=BG checkBtn.Font=Enum.Font.GothamBlack checkBtn.TextSize=18
    checkBtn.BorderSizePixel=0
    Instance.new("UICorner",checkBtn).CornerRadius=UDim.new(0,8)
    local pW,pH,dSz=46,24,18
    local track=Instance.new("Frame") track.Size=UDim2.new(0,0,0,0) track.Visible=false
    local dot=Instance.new("Frame") dot.Size=UDim2.new(0,0,0,0) dot.Visible=false
    toggleStates[name]={track=track,dot=dot,state=initState,dotSz=dSz,checkBtn=checkBtn}
    ...
    checkBtn.MouseButton1Click:Connect(function()
        ...
        checkBtn.BackgroundColor3=ns and WHITE or OFF_CLR
        checkBtn.Text=ns and "✓" or ""
...
local function updateToggle(name,state)
    ...
    t.state=state
    if t.checkBtn then
        t.checkBtn.BackgroundColor3=state and WHITE or OFF_CLR
        t.checkBtn.Text=state and "✓" or ""
    end
...
local function makeNumInput(parent,labelText,defaultVal,order,onChanged)
    ...
    box.BackgroundColor3=Color3.fromRGB(25,25,25) box.Text=tostring(defaultVal)
...
do
    ...
    dropBtn.Size=UDim2.new(1,0,0,40) dropBtn.BackgroundColor3=Color3.fromRGB(20,20,20)
...
do
    ...
    tpDownBtn.Size=UDim2.new(1,0,0,40) tpDownBtn.BackgroundColor3=Color3.fromRGB(20,20,20)
...
makeSection(SetFrame,"  AUTO TP DOWN",15)
makeToggle(SetFrame,"Auto TP Down",16,function(v)
    autoTPDownEnabled=v if v then startAutoTPDown() else stopAutoTPDown() end
end)
makeNumInput(SetFrame,"Altura do Pulo (Y)",20,17,function(v) autoTPDownTargetY=math.max(1,v) end)
makeSection(SetFrame,"  CONFIG",18)

do
    local saveBtn=Instance.new("TextButton",SetFrame)
    saveBtn.Size=UDim2.new(1,0,0,40) saveBtn.BackgroundColor3=Color3.fromRGB(20,20,20)
    saveBtn.Text="💾  SAVE CONFIG" saveBtn.Font=Enum.Font.GothamBlack saveBtn.TextSize=13
    saveBtn.TextColor3=WHITE saveBtn.BorderSizePixel=0 saveBtn.LayoutOrder=19
...
do
    ...
    bindHint.Size=UDim2.new(1,0,0,36) bindHint.BackgroundColor3=Color3.fromRGB(15,8,30)
    bindHint.BackgroundTransparency=0 bindHint.BorderSizePixel=0
    bindHint.Text="Clique e pressione tecla ou botão do controle. CTRL = limpar."
    bindHint.TextColor3=Color3.fromRGB(200,200,200) bindHint.Font=Enum.Font.Gotham
...
for idx,entry in ipairs(bindList) do
    ...
    keyBtn.MouseButton1Click:Connect(function()
        ...
        changingKeybind=keyName keyBtn.Text="Pressione..." keyBtn.TextColor3=Color3.fromRGB(200,200,200)
...
do
    ...
    PBC.BackgroundColor3=Color3.fromRGB(15,15,15) PBC.BackgroundTransparency=0.08 PBC.BorderSizePixel=0
    ...
    task.spawn(function()
        ...
        grad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,ACCENT),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(20,10,50)),ColorSequenceKeypoint.new(1,ACCENT)})
    ...
    rl.Text="Radius:" rl.TextColor3=Color3.fromRGB(200,200,200) rl.Font=Enum.Font.GothamBold rl.TextSize=11 rl.ZIndex=3
    ...
    pt.BackgroundColor3=Color3.fromRGB(40,40,40) pt.BorderSizePixel=0 pt.ZIndex=2
...
do
    ...
    -- BOTAO DESYNC (abaixo do TP DOWN)
    local desyncEnabled = false
    local desyncDidReset = false
    local function resetCharacterDesync()
        local char = player.Character if not char then return end
        local hum = char:FindFirstChildOfClass("Humanoid") if hum then hum.Health = 0 end
    end
    makeMob("DESYNC",col1,startY+gap*3,nil,function()
        desyncEnabled = not desyncEnabled
        if desyncEnabled then
            raknet.desync(true)
            if not desyncDidReset then desyncDidReset = true resetCharacterDesync() end
        else
            raknet.desync(false)
        end
    end)

    -- BOTAO TAUNT (ao lado do DESYNC)
    makeMob("TAUNT",col2,startY+gap*3,nil,function()
...
do
    ...
    OCGui.Name="VoidDuelsOpenClose" OCGui.ResetOnSpawn=false OCGui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling
    local OBtn=Instance.new("TextButton",OCGui)
    OBtn.Size=UDim2.new(0,52,0,52) OBtn.Position=UDim2.new(0,10,0.5,-26)
    OBtn.BackgroundColor3=Color3.fromRGB(15,15,15) OBtn.Text="⚔️" OBtn.TextSize=26
...
if espEnabled then enableESP() end
applyFOV()
print("VOID DUELS loaded — https://discord.gg/4UmHFVpN")
